...
def sample_by_bald_class_easiness(tokenizer, X, y_mean, y_var, y, num_samples, num_classes, y_T, alpha, cb_loss=True, true_label = None, active_learning= False, active_number = 16, uncert = True, up_scale= True):
	...
	unselected_input_ids, unselected_token_type_ids, unselected_attention_mask, unselected_mask_pos, unselected_y_s = [], [], [], [], []
	not_sample = 0
	...
	for label in range(num_classes):
		...
		exclude_indices = [i for i in range(len(X_input_ids)) if i not in indices]
		
		# indices에 해당하지 않는 값들을 선택합니다.
		unselected_X_s_input_ids = [X_input_ids[i] for i in exclude_indices]
		# X_s_token_type_ids = [X_token_type_ids[i] for i in exclude_indices]
		unselected_X_s_attention_mask = [X_attention_mask[i] for i in exclude_indices]
		...
		if "token_type_ids" in X.features:
			...
			unselected_X_s_token_type_ids = [X_token_type_ids[i] for i in exclude_indices]
			unselected_token_type_ids.extend(unselect_X_s_token_type_ids)
			
		if "mask_pos" in X.features:
			X_s_mask_pos.extend(X_mask_pos[indices])
			unselected_X_mask_pos = [X_mask_pos[i] for i in exclude_indices]
			unselected_mask_pos.extend(unselect_X_mask_pos)
		
		y_s.extend(y_[indices])
		unselected_y_s = [y_[i] for i in exclude_indices]
